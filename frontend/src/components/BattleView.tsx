        ...
        {startConfirm && !battleStarted && opponent.type && (
          <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4 overflow-y-auto py-4">
            <div className="w-full max-w-md rounded-2xl border border-white/[0.16] bg-[#0E1016]/95 p-6 shadow-[0_25px_70px_rgba(0,0,0,0.55)] my-auto">
              ...
              <div className="mt-3">
                <div className="text-[10px] uppercase tracking-widest text-gray-500 mb-2">Cases in battle</div>
                <div className="space-y-1.5 max-h-[200px] overflow-y-auto custom-scrollbar pr-1">
                  {selectedCases.map((caseData, idx) => (
                    <div key={`confirm-${caseData.id}-${idx}`} className="flex items-center gap-2.5 rounded-lg border border-white/[0.06] bg-black/30 px-3 py-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[9px] text-gray-500">
                          ${(caseData as any).tokenTicker || caseData.currency} · 1 token = {(caseData as any).tokenPrice || '?'} ₮
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="text-[11px] font-black text-web3-accent">{format2(caseData.price)} ₮</div>
                        <div className="text-[9px] text-gray-500">{caseData.possibleDrops?.length || 0} drops</div>
                      </div>
                    </div>
                  ))}
                </div>
